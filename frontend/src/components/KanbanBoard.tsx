...
import { columnAccentColor, moveCard, type BoardData } from "@/lib/kanban";
...
export const KanbanBoard = () => {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <main className="relative flex min-h-screen w-full flex-col gap-10 px-6 pb-16 pt-12">
            ...
            {board.columns.map((column) => (
              ...
              >
                <span
                  ...
                  style={{
                    backgroundColor: columnAccentColor(board.columns, column.id),
                  }}
                />
                {column.title}
              </div>
        ...
        >
          <section className="grid grid-cols-[repeat(5,minmax(220px,1fr))] gap-6 overflow-x-auto pb-2">
            {board.columns.map((column) => (
              <KanbanColumn
                key={column.id}
                column={column}
                cards={column.cardIds.map((cardId) => board.cards[cardId])}
                accentColor={columnAccentColor(board.columns, column.id)}
