...
- `src/components/ChatSidebar.tsx` — AI chat sidebar rendered alongside the board once it's loaded. Owns its own `messages` history (not lifted to `KanbanBoard`); on send, `POST /api/ai/chat` with `{board, message, history}` and appends the reply. No optimistic update for the board — unlike the drag/rename/add/delete mutations, there's no way to predict what the AI will do, so it just calls `onBoardUpdate(data.board)` (wired to `KanbanBoard`'s `setBoard`) with the server's authoritative response once it arrives. Test hooks: `data-testid="chat-input"`/`"chat-send"`/`"chat-messages"`/`"chat-message-user"`/`"chat-message-assistant"`/`"chat-error"`. Its `<h2>` is deliberately "Chat with the assistant", not anything containing "Kanban Studio" — the board header's `<h1>Kanban Studio</h1>` is elsewhere on the same page and `getByRole("heading", {name: "Kanban Studio"})` substring-matches by default, so a second heading containing that phrase breaks existing tests with a strict-mode violation (hit this once — see git history).
