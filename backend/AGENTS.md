...
- `app/ai.py` — `ask(prompt) -> str`, a thin wrapper around the `anthropic` SDK. Constructs `anthropic.Anthropic()` (reads `ANTHROPIC_API_KEY` from the environment) fresh per call rather than once at import/startup, so a missing/invalid key can't crash the app at boot and env changes (e.g. in tests) take effect immediately. `POST /api/ai/ask` (behind `require_user`, like the board routes) wraps it — model is `claude-sonnet-5` per CLAUDE.md. Also has `chat(board, message, history) -> ChatReply` for the Kanban-aware assistant: `RenameColumnAction`/`AddCardAction`/`EditCardAction`/`DeleteCardAction`/`MoveCardAction` are a Pydantic union, `ChatReply = {reply: str, action: BoardAction | None}` is passed as `output_format` to `client.messages.parse(...)` (structured outputs) so Claude's response is always one of these shapes or `parsed_output is None` (refusal/incomplete — falls back to `FALLBACK_REPLY`). `apply_action(conn, username, action)` dispatches to the matching `board.py` function to actually persist it. `POST /api/ai/chat` calls `chat()`, then `apply_action()` if an action came back, wrapped in `except HTTPException: pass` — if the model hallucinated a card/column id or an invalid edit, `board.py`'s existing validation catches it and the turn just returns the reply with the board unchanged, rather than 500ing.
...
- `tests/test_ai_chat.py` — two *live* tests matching the plan's own examples ("How many columns..." → no action, board unchanged; "Move the roadmap card to Done" → `card-1` ends up in `col-done`), plus mocked tests (`_stub_anthropic` monkeypatches `app.ai.anthropic.Anthropic` with a fake `.messages.parse(...)` returning a canned `ChatReply` or `None`) for deterministic coverage of: an action applying and matching the returned board, an unparseable/refused response, and an action referencing a nonexistent card — all three assert a 200 with a sensible reply and, for the latter two, an unchanged board.
...
- No chat UI wired up yet — `/api/ai/chat` is fully functional but only exercised via tests/curl (Part 10)
